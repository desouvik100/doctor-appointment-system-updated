 ...
 * Route: /api/appointments/clinic/:clinicId (filtered) or direct by ID via user route
 */
export const getAppointmentById = async (appointmentId) => {
  // Use the clinic appointments endpoint and filter, or use user route
  const response = await apiClient.get(`/appointments/user/${appointmentId}`).catch(async () => {
    // Fallback: try getting from clinic appointments
    return null;
  });
...
/**
 * Get patient visit history (appointments for a patient at this clinic)
 */
export const getPatientVisits = async (patientId) => {
  const response = await apiClient.get(`/appointments/user/${patientId}`);
  return response.data;
};
...
export const shiftCheckIn = async (clinicId, staffId) => {
  const response = await apiClient.post('/staff-management/attendance/check-in', {
    clinicId,
    staffId,
...
export const shiftCheckOut = async (clinicId, staffId) => {
  const response = await apiClient.post('/staff-management/attendance/check-out', {
    clinicId,
    staffId,
...
export const getTodayAttendance = async (clinicId, staffId) => {
  const today = new Date().toISOString().split('T')[0];
  const response = await apiClient.get(`/staff-management/attendance/clinic/${clinicId}`, {
    params: { staffId, startDate: today, endDate: today },
  });
  return response.data;
};
...
export const getDailySummary = async (clinicId) => {
  // Reuse today's appointments endpoint
  const response = await apiClient.get(`/receptionists/appointments/${clinicId}`);
  return response.data;
};
...
export default {
  ...
  getDoctorDetails,
  // Single appointment + patient visits
  getAppointmentById,
  getPatientVisits,
  // Shift / Attendance
  shiftCheckIn,
